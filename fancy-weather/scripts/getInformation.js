async function getInformation(search) {
  try {
    ...
    let pora;
    if (datMes === '12' || datMes === '1' || datMes === '2') { pora = 'winter'; }
    if (datMes === '3' || datMes === '4' || datMes === '5') { pora = 'spring'; }
    if (datMes === '6' || datMes === '7' || datMes === '8') { pora = 'summer'; }
    if (datMes === '9' || datMes === '10' || datMes === '11') { pora = 'autumn'; }
    let times;
    if (num >= 6) { times = 'morning'; }
    if (num >= 12 && num < 18) { times = 'sunny day'; }
    if (num >= 18 && num <= 24) { times = 'evening'; }
    if (num < 6) { times = 'night'; }

    if (nachalo === 0) { getBackground(times, pora); } else {
      setTimeout(() => {
